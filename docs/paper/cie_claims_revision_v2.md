...
| C8b | `SUPPORTED_WITH_CONDITION` | 旧南宁 ported DH 批次已定级 `INVALIDATED_ZERO_THROUGH_STATE_MACHINE_BUG`，其中 16 个已结束格也不得用于性能结论；修正版跨图随机比较的实际完成状态与逐指标方向以[修复后报告](../../outputs/reports/feng_cie_dh_repair_comparison_20260905.md)为准 | 180 格=2 maps×3 loads×10 seeds×3 methods；经 map2 行为回归允许复用的 150 个有效格与修复后另跑的 30 个南宁 DH 格严格分开。只有每格 10/10 paired seeds、最终 source/class 身份和全矩阵完整性门通过后才陈述最终随机结论；partial/ported 均为 secondary executable diagnostics | [科学有效性旁车](../../outputs/runtime/cie_external_baseline_robustness/scientific_validity_20260905.json)；[修复后报告](../../outputs/reports/feng_cie_dh_repair_comparison_20260905.md) |
