...
| C8b | `NOT_IDENTIFIED` | 2026-09-05 关机检查点为 164/180：map2 全部完成，南宁 ported DH 仅 14/30；完整随机跨图/跨负载方向仍只能在最终 SHA、每格 10/10 paired seeds 和全矩阵 180/180 strict normalized cells 门通过后陈述 | 180 格=2 maps×3 loads×10 seeds×3 methods；map2 partial 与南宁 ported 均为 secondary executable diagnostics；当前 1× 7 seeds、1.75× 4 seeds 和 2× 3 seeds 只作检查点，不得进入最终胜负 | `cie_external_baseline_checkpoint_20260905.md`; `cie_external_baseline_robustness.md` |
