"""V6 parser V2: preserve native runs while correctly parsing censored N/A clocks."""
...
def finite_or_none(value):
    if value is None or (isinstance(value, str) and value.strip() in ("", "N/A", "NaN", "nan")):
        return None
    parsed = float(value)
    require(math.isfinite(parsed), "non-finite native clock: " + repr(value))
    return parsed
...
def run_spec(spec_path, output_dir):
    ...
    spec = read(spec_path)
    require(not spec.get("native_recovery_from"), "recovery-only spec must use retained native evidence, never rerun Java")
