"""Run the isolated HCA A* time-label repair on either of two frozen 1x workloads.
...
METHOD = "HCA_TIME_LABEL_REPAIR_V2"
SCHEMA = "czr005.hca_time_label_repair_result.v1"
PROTOCOL = ROOT / "docs/baselines/hca_time_label_repair_v2_two_cell_protocol_20260906.md"
ISOLATED = ROOT / "benchmarks/java/hca_time_label_repair_v2"
WRAPPER = ISOLATED / "HcaSegmentIdentityBenchmark.java"
...
def verify_build(classes_dir: Path, java: str | None = None, javac: str | None = None) -> dict:
    ...
    require(value.get("schema") == "czr005.hca_time_label_repair_build.v1" and value.get("method") == METHOD,
...
def compile_production(classes_dir: Path, java: str, javac: str) -> dict:
    ...
    value = {"schema": "czr005.hca_time_label_repair_build.v1", "method": METHOD,
    ...
    for raw, legs in by_raw.items():
        if "storage_in" in legs:
            ...
            if second["processed_attempt_epoch"] is not None and (first["finish_epoch"] is None or
                ...
                overlap.append(raw)
    return lifecycle, {"schema": "czr005.hca_time_label_repair_population_audit.v1", "status": "PASS",
...
def normalize_cell(identity_path: Path, native_dir: Path, *, identity_audited: bool = False) -> dict:
    ...
    require(build.get("method") == METHOD and build.get("schema") == "czr005.hca_time_label_repair_build.v1",
...
def archive_cell(native_dir: Path) -> dict:
    ...
    value = {"schema": "czr005.hca_time_label_repair_archive.v1", "method": METHOD, "files": records,
...
def validate_output_directory(output: Path) -> Path:
    ...
    for base in (ROOT / "outputs/runtime", ROOT / "tmp"):
        if resolved.is_relative_to(base):
            ...
            allowed = bool(relative.parts) and relative.parts[0].startswith("hca_time_label_repair_v2_")
    require(allowed, "use a new outputs/runtime/hca_time_label_repair_v2_* or tmp/hca_time_label_repair_v2_* directory")
...
def cleanup_epoch_scratch(output: Path) -> dict:
    ...
    if scratch.exists():
        scratch.rmdir()
    result = {"schema": "czr005.hca_time_label_repair_scratch_cleanup.v1",
...
def run_cell(identity_path: Path, output: Path, classes_dir: Path, java: str, javac: str, *,
    ...
    require(identity["map"] in ("map2", "nanning") and float(identity["load_factor"]) == 1.0
            and int(identity["seed"]) == 104729, "only the two authorized 1x seed 104729 cells may run")
    expected_identity = ROOT / "data/processed/workloads/cie_external_robustness" / (identity["map"] + "_1p00x") / "seed_104729/identity.json"
    require(identity_path.resolve() == expected_identity.resolve(), "use the existing frozen paired workload")
    output, classes_dir = validate_output_directory(output), Path(classes_dir).resolve(strict=True)
    expected_output = ROOT / "outputs/runtime/hca_time_label_repair_v2_20260906" / (identity["map"] + "_1p00x") / "seed_104729/hca_time_label_repair_v2"
    require(output == expected_output.resolve(), "exactly one fixed output per authorized cell; no extra repeats")
    ...
    try:
        ...
        status = {"schema": "czr005.hca_time_label_repair_run.v1", "method": METHOD, "status": "running",
