...
def audit_payload(prepared: Prepared, payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    # The native active-state flag also requires a quiescent terminal state,
    # and the full-protocol flag further requires an untruncated debug log.
    # At a fixed horizon valid in-flight grants may remain. Keep both flags
    # exactly as observed and independently require conservation/bijection.
    structural_integrity = (summary.get("merge_grant_conservation_holds") is True
    ...
    quiescent = (summary.get("merge_grant_final_active_unconsumed") == 0
                 and summary.get("merge_grant_outstanding_request_count") == 0)
    active_integrity = structural_integrity and quiescent
    ...
    gates = {
        ...
        "native_merge_structural_state": structural_integrity,
        "complete_population_has_no_active_grants": quiescent or completed < len(bags),
        "active_grant_count_nonnegative": isinstance(summary.get("merge_grant_final_active_unconsumed"), int)
                                          and summary["merge_grant_final_active_unconsumed"] >= 0,
        "pending_grant_count_nonnegative": isinstance(summary.get("merge_grant_outstanding_request_count"), int)
                                           and summary["merge_grant_outstanding_request_count"] >= 0,
    ...
    return {
        ...
        "trace": {"decision_trace_truncated": summary.get("decision_trace_truncated"),
                  ...
                  "merge_grant_structural_integrity_pass": structural_integrity,
                  "terminal_state_quiescent": quiescent,
                  "active_inflight_grants_at_horizon": summary.get("merge_grant_final_active_unconsumed"),
                  "pending_requests_at_horizon": summary.get("merge_grant_outstanding_request_count"),
...
def load_completed(output_dir: Path, *, prepared: Prepared | None = None) -> dict[str, Any]:
    ...
    if _encode(result["provenance"]) != _encode(prepared.provenance):
