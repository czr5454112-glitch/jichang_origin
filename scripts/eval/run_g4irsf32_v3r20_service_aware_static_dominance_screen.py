#!/usr/bin/env python3
"""Run the frozen V3R20 service-aware static-dominance core screen.
...
existing three-arm execution and gate logic.  Injected execution neither
...
SCHEMA = "czr005.g4irsf32.v3r20.service_aware_static_dominance_screen.v1"
PROTOCOL_ID = "G4IRSF32_V3R20_SERVICE_AWARE_STATIC_DOMINANCE_20260829"
MEASUREMENT_REQUIRED = (
    "MEASUREMENT_REQUIRED_V3R20_SERVICE_AWARE_STATIC_DOMINANCE"
)
NO_GO = "NO_GO_V3R20_SERVICE_AWARE_STATIC_DOMINANCE"
PASS = MEASUREMENT_REQUIRED

PREREGISTRATION_PATH = (
    ROOT
    / "docs/G4IRSF32_v3r20_service_aware_static_dominance_preregistration.md"
)
...
ARMS = ("off", "candidate_a", "candidate_a_static_dominance")
OLD_SCORER = previous.OLD_SCORER
CANDIDATE_SCORER = "S4_service_aware_static_dominance_rule_only"
...
ServiceAwareStaticDominanceScreenError = previous.SourceReleaseTieScreenError
...
def _resolve_binary(binary: Path) -> Path:
    """Bind formal execution to an explicit V3R20 build without loading it."""

    resolved = binary.resolve(strict=True)
    if not resolved.is_file():
        raise ServiceAwareStaticDominanceScreenError(
            "V3R20 binary must be a file"
        )
    if "build_g32_v3r20" not in {part.lower() for part in resolved.parts}:
        raise ServiceAwareStaticDominanceScreenError(
            "service-aware-static-dominance binary must come from "
            "build_g32_v3r20"
        )
    return resolved
...
def _build_arm_requests(
    ...
    *,
    ...
    candidate_a_static_dominance = copy.deepcopy(candidate_a)
    candidate_a_static_dominance["scorer_mode"] = CANDIDATE_SCORER
    expected = copy.deepcopy(candidate_a)
    expected["scorer_mode"] = CANDIDATE_SCORER
    if candidate_a_static_dominance != expected:
        raise ServiceAwareStaticDominanceScreenError(
            "Candidate A static-dominance request has an unregistered delta"
        )
    if candidate_a_static_dominance.get("event_trace_limit") != EVENT_TRACE_LIMIT:
        raise ServiceAwareStaticDominanceScreenError(
    ...
    return (
        {
            ...
            "candidate_a_static_dominance": candidate_a_static_dominance,
        },
        runtime_rows,
        rejected,
    )
...
# The existing executor enforces native scorer identity, Candidate A mode,
# safety extraction, and the frozen V3R13 cohort contract for arbitrary arms.
...
def _run_case(
    ...
    *,
    ...
    return shared._evaluate_three_arm_case(
        ...
        eligible_raw_task_ids=eligible_raw_task_ids,
        eligible_target_segment_ids=eligible_target_segment_ids,
        primary_arm="candidate_a_static_dominance",
...
def run_screen(*, executor: Executor, binary: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "arms": list(ARMS),
        "primary_comparison": "candidate_a_static_dominance/off",
        "attribution_only_comparison": (
            "candidate_a_static_dominance/candidate_a"
        ),
        "thresholds": {
...
def render_report(result: Mapping[str, Any]) -> str:
    lines = [
        ...
        "",
        "Primary comparison: `candidate_a_static_dominance / off`. "
        "Historical Candidate A attribution is diagnostic only.",
        "",
        ...
        [
            ...
            *(
            ...
            (
                ...
                "wait/idle measurement support is required before any Stage 3 "
                "decision."
                if result.get("core_screen_pass")
                else "Service-aware static dominance is NO-GO; Stage 3 remains "
                "blocked."
            ),
            "",
...
def write_evidence(
    ...
    *,
    ...
    """Publish only the two registered append-only V3R20 outputs."""

    if json_path.exists() or markdown_path.exists():
        raise FileExistsError(
            "V3R20 service-aware-static-dominance outputs are append-only"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            "SERVICE_AWARE_STATIC_DOMINANCE_SCREEN_RUNNER_ERROR: "
