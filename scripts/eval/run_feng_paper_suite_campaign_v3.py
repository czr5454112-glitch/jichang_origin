"""Continue the frozen V6 reduced paper suite in qualified, resumable stages.
...
PARENT_PLAN = ROOT / "outputs/evidence/paper_suite_20260907/frozen_plan_v5/plan.json"
PARENT_PLAN_SHA = "00a95b47298fb1aac808bec9275f6d9b3c28d340859017c0667e39ff736b9719"
SCOPE_CONTRACT = ROOT / "configs/eval/feng_paper_suite_reduced_20260907.json"
BINARY_SHA = "38b07ddfbc661c5243ab62a4963d84514405a87b11afa41538a94a42d2665b20"
REPORTER = ROOT / "scripts/eval/report_feng_paper_suite_v2.py"
...
FAULT_SEEDS = (104729, 130363, 155921)
...
def verify_scope(plan):
    scope_ref = plan["scope_contract"]
    require(Path(scope_ref["path"]).resolve() == SCOPE_CONTRACT, "unregistered reduced scope contract")
    bound(SCOPE_CONTRACT, scope_ref["sha256"])
    scope = pool.read(SCOPE_CONTRACT)
    require(scope.get("schema") == "czr005.feng_paper_suite.reduced_scope.v1"
            and scope.get("cell_count") == 576 and scope.get("family_counts") == {"base": 480, "all_day_fault": 96}
            and scope.get("execution_order") == ["base", "all_day_fault"]
            and scope["all_day_fault"]["scenario_indices"] == list(FAULT_SCENARIOS)
            and scope["all_day_fault"]["seeds"] == list(FAULT_SEEDS), "scope contract differs from the authorized matrix")
    reduction = plan["scope_reduction"]
    require(Path(reduction["source_plan_path"]).resolve() == PARENT_PLAN
            and reduction["source_plan_sha256"] == PARENT_PLAN_SHA, "reduction is not bound to the frozen V5 parent")
    bound(PARENT_PLAN, PARENT_PLAN_SHA)
    original = pool.read(PARENT_PLAN)
    retained = [cell for cell in original["cells"] if pool.family(cell["family"]) == "base" or
                (cell["scenario_index"] in FAULT_SCENARIOS and cell["seed"] in FAULT_SEEDS)]
    require(plan["cells"] == retained, "retained cells changed from the V5 parent")
    removed = {cell["cell_id"] for cell in original["cells"]} - {cell["cell_id"] for cell in retained}
    require(reduction.get("user_approved") is True and reduction.get("retained_cell_entries_exactly_equal") is True
            and reduction.get("normal_first") is True and reduction.get("removed_count") == 1184
            and len(reduction["removed_cell_ids"]) == 1184 and set(reduction["removed_cell_ids"]) == removed,
            "scope-reduction provenance differs")
    return [ref(SCOPE_CONTRACT), ref(PARENT_PLAN)]
...
def stage_cells(plan):
    ...
    require(len(base) == 16 and len(fault) == 8 and len(plan["cells"]) == 576, "registered stage cell counts differ")
    require(plan.get("family_counts") == {"base": 480, "all_day_fault": 96}, "wrong reduced suite family population")
    require(plan.get("execution_order") == ["base", "all_day_fault"], "all base cells must precede fault dispatch")
    faults = pool.select_cells(plan, {"family": ["all_day_fault"]})
    expected = {(method, map_name, load, 2.5, seed, scenario)
                for method in (pool.NEW_G31, pool.HCA) for map_name in ("map2", "nanning")
                for load in (1, 2) for seed in FAULT_SEEDS for scenario in FAULT_SCENARIOS}
    actual = {tuple(cell[k] for k in ("method", "map", "load_factor", "speed_mps", "seed", "scenario_index"))
              for cell in faults}
    require(len(faults) == 96 and actual == expected, "fault coordinates differ from the selected four scenarios and three seeds")
    require(len(pool.select_cells(plan, {"family": ["base"]})) == 480, "all 480 base cells must remain")
    return base, fault
...
class Campaign:
    def __init__(self, plan_path, wait_seconds=WAIT_SECONDS):
        ...
        require(pool.sha(self.plan_path) == PLAN_SHA, "only the fixed V6 plan is authorized")
        self.plan = pool.load_plan(self.plan_path)
        self.scope_references = verify_scope(self.plan)
        self.base, self.fault = stage_cells(self.plan)
        self.all_base = pool.select_cells(self.plan, {"family": ["base"]})
        self.all_fault = pool.select_cells(self.plan, {"family": ["all_day_fault"]})
        ...
        self.status = {"schema": SCHEMA, "status": "STARTING", "run_id": self.run_id, "pid": os.getpid(),
            ...
            "execution_order": ["base", "all_day_fault"], "expected_cells": 576,
    ...
    def unchanged(self):
        ...
        for item in self.scope_references:
            bound(item["path"], item["sha256"])
    ...
    def report(self):
        ...
        try:
            ...
            if self.status["status"] == "COMPLETE":
                require(snapshot["status"] == "COMPLETE" and snapshot.get("expected_cells") == 576,
                        "all pools returned but the report is not a complete 576-cell result")
    ...
    def run(self):
        ...
        try:
            ...
            normal = self.wait_for("phase2_wait_normal_full_2x", self.normal2_probe)
            self.status["prerequisites"].append(normal)
            self.save()
            self.execute_stage("phase3_base_all480_resume_verified", self.all_base)
            self.execute_stage("phase4_fault_seed104729_load1_scenarios1_2", self.fault)
            self.execute_stage("phase5_fault_all96_resume_verified", self.all_fault)
...
def main(argv=None):
    ...
    if args.check_only:
        ...
        print(json.dumps({"status": "PRECHECK_PASS_NOT_EXECUTED", "plan_sha256": PLAN_SHA,
                          "stage_counts": [len(campaign.base), len(campaign.all_base), len(campaign.fault), len(campaign.all_fault)],
                          "expected_cells": 576, "execution_order": ["base", "all_day_fault"],
