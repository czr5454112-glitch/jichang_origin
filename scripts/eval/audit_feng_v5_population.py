"""Audit complete-process V5 populations on both maps without inspecting running outputs.
...
OUTPUT = ROOT / "outputs/evidence/feng_dh_boundary_clearance_v5_20260905/population_audit.json"
...
def audit_cell(native: Path, status: dict, load: float, seed: int) -> dict:
    ...
    require(identity["method"] == "FENG_DH_BOUNDARY_CLEARANCE_V5", "runner method mismatch")
    ...
    require(workload["map"] in external.MAPS and float(workload["load_factor"]) == load
    ...
    summary = summaries[0]
    require(summary["status"] in {"COMPLETE", "HORIZON_REACHED", "DEADLOCK"}, "unexpected terminal status")
    ...
    require(physical_through_stops >= 0, "negative residual service/clearance stops")
    ...
    return {"map": workload["map"], "load_factor": load, "seed": seed, "status": "PASS",
            ...
            "final_active_segment_count": active, "service_and_clearance_stopped_ticks_by_counter_partition": physical_through_stops,
            "counter_sums": dict(totals), "native_sha256": hashes, "workload_identity_sha256": reference["sha256"]}


# Called by export_feng_v5_campaign after clean-process identity validation.
