...
restarted or overwritten. One pool lock covers the entire campaign root. The DH parser is selected only
from the two explicitly allowed, SHA-bound runner versions in the frozen plan.
"""
...
DH_RUNNER_MODULES = {
    ROOT / "scripts/eval/run_feng_dh_paper_suite.py": "scripts.eval.run_feng_dh_paper_suite",
    ROOT / "scripts/eval/run_feng_dh_paper_suite_v2.py": "scripts.eval.run_feng_dh_paper_suite_v2",
}
...
def selected_runner(cell: dict) -> Path:
    """Select a SHA-bound, explicitly allowed runner; never an arbitrary command."""
    method = cell["method"]
    require(method in RUNNERS, "method is outside the four-method paper suite")
    allowed = DH_RUNNER_MODULES if method == DH else (RUNNERS[method],)
    selected = Path(cell["runner_path"]).resolve(strict=True)
    require(selected in {path.resolve() for path in allowed},
            "plan runner does not match the fixed method-to-runner whitelist")
    require(sha(selected) == cell["runner_sha256"], "bound runner changed")
    return selected


def verify_bound_cell(cell: dict) -> dict:
    """Recheck small immutable identities immediately before each use."""
    selected_runner(cell)
...
def verify_dh(cell: dict, spec: dict) -> None:
    ...
    require(cell["method"] == DH, "DH verification requires the DH method")
    runner = selected_runner(cell)
    module_name = next(name for path, name in DH_RUNNER_MODULES.items() if path.resolve() == runner)
    ...
    if "native_recovery_from" in spec:
        require(module_name == "scripts.eval.run_feng_dh_paper_suite_v2", "recovery requires the V2 DH parser")
        recovery_helper = importlib.import_module("scripts.eval.recover_feng_dh_paper_suite_v2")
        source, recovered_output, _, _, source_status = recovery_helper.validate_source(spec, output)
        require(recovered_output == output.resolve(), "recovery validated a different output")
        origin = spec["native_recovery_from"]
        for name in recovery_helper.FILES:
            target = output / ("source_runner_status.json" if name == "runner_status.json" else name)
            require(target.resolve().parent == output.resolve()
                    and sha(target) == origin["files_sha256"][name], "recovered native copy drift: " + name)
        provenance_path = output / "native_recovery.json"
        provenance = read(provenance_path)
        require(sha(provenance_path) == result["native_recovery_sha256"]
                and status.get("recovery") == provenance
                and status.get("normalized_result_sha256") == sha(output / "normalized_result.json"),
                "recovery provenance/result binding drift")
        require(provenance.get("schema") == "czr005.dh_parser_recovery.v2"
                and provenance.get("native_reexecuted") is False
                and result.get("native_reexecuted") is False and status.get("native_reexecuted") is False
                and provenance.get("original_native_bytes_unchanged") is True,
                "recovery must preserve the original native execution")
        require(provenance.get("source") == origin
                and provenance.get("source_spec_sha256") == origin["spec_sha256"]
                and provenance.get("native_runner_sha256") == status.get("native_runner_sha256") == origin["runner_sha256"]
                and provenance.get("original_native_command") == status.get("command") == source_status["command"]
                and provenance.get("native_started_at_unix") == status.get("started_at_unix") == source_status["started_at_unix"]
                and provenance.get("native_wall_seconds") == status.get("wall_seconds") == source_status["wall_seconds"],
                "recovery original execution provenance drift")
        require(Path(provenance["normalizer_path"]).resolve() == runner
                and provenance["normalizer_sha256"] == cell["runner_sha256"]
                and Path(provenance["recovery_tool_path"]).resolve() == Path(recovery_helper.__file__).resolve()
                and provenance["recovery_tool_sha256"] == sha(Path(recovery_helper.__file__)),
                "recovery parser/helper identity drift")
...
def execute_cell(cell: dict, record_dir: Path, timeout_seconds: float, abort: threading.Event) -> dict:
    ...
    try:
        ...
        command = [sys.executable, str(selected_runner(cell)), "run",
