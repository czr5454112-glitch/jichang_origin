...
def _random_topology_pibt_stress_rows() -> list[dict[str, str]]:
    path = SOURCE_TABLES["phase9_random_topology_pibt_stress_sweep"]
    rows: list[dict[str, str]] = []
    for source in _read_csv(path):
        python_conflicts = int(float(source["python_conflicts"]))
        cpp_conflicts = int(float(source["cpp_conflicts"]))
        notes = (
            f"Random DAG-like topology layers={source['layers']}; "
            f"source_mode={source['source_mode']}; goal_mode={source['goal_mode']}; "
            f"branch={source['branch_probability']}; shortcut={source['shortcut_probability']}; "
            f"sources={source['source_histogram']}; goals={source['goal_histogram']}."
        )
        rows.append(
            _blank_row(
                evidence_group="random_topology_pibt_stress_sweep",
                source_table=_source_name(path),
                source_scope="random DAG-like synthetic ICS topologies and task-source distributions",
                case=source["scenario"],
                policy_or_baseline="pibt_active_bag_replay",
                engine="python_cpp",
                max_tasks=_int_text(source["task_count"]),
...
def _summarize(rows: list[dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "random_topology_pibt_stress_row_count": len(random_topology_rows),
        "random_topology_pibt_stress_safety_pass": all(
            row["safety_pass"] == "True" for row in random_topology_rows
        ),
...
def write_report(rows: list[dict[str, str]]) -> None:
    ...
    for row in random_topology_rows:
        lines.append(
            "| {case} | {max_tasks} | {fault_label} | {config_label} | {planned_count} | {decision_count} | "
            "{post_shield_conflicts} | {cpp_decision_speedup} | {strict_parity_pass} | {notes} |".format(
                **{**row, "fault_label": _fault_label(row), "config_label": _config_label(row)}
            )
        )
    ...
    lines.extend(
        [
            ...
            if summary["random_topology_pibt_stress_parity_pass"]
            else "- random topology PIBT stress Python/C++ parity rows pass: FAIL",
