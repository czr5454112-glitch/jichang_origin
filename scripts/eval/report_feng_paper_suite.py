"""Read-only, complete-plan progress/results snapshots for the Feng paper suite.
...
SCHEMA = "czr005.feng_paper_suite.report_snapshot.v1"
...
def load_plan(path, reader):
    ...
    require(len(cells) == 1760, "the report requires all 1760 registered cells")
    require(len({c["cell_id"] for c in cells}) == 1760, "duplicate cell ID")
    require(len({str(Path(c["output_dir"]).resolve()) for c in cells}) == 1760, "duplicate output")
    seeds = sorted({c["seed"] for c in cells})
    require(len(seeds) == 10 and all(type(s) is int for s in seeds), "exactly ten common seeds required")
    expected = set()
    for m, load, speed, seed, method in itertools.product(
            ("map2", "nanning"), (1, 2), (1.5, 2.5, 3.0), seeds, METHODS):
        expected.add(("base", m, load, speed, 0, seed, method))
    for m, load, scenario, seed, method in itertools.product(
            ("map2", "nanning"), (1, 2), range(1, 17), seeds, (G31, HCA)):
        expected.add(("all_day_fault", m, load, 2.5, scenario, seed, method))
    actual = [tuple(c[k] for k in (*COORD, "method")) for c in cells]
    require(len(set(actual)) == 1760 and set(actual) == expected, "registered coordinate set differs")
...
def pair_cells(rows):
    ...
    for coordinate, methods in sorted(by_coord.items()):
        ...
        for baseline in (HCA, DH, TARAU):
            ...
            pairs.append(pair)
    require(len(pairs) == 1000, "all 1000 registered G31/baseline seed pairs required")
    return pairs
...
def group_results(rows, pairs, seeds):
    ...
    for population, keys, target in ((rows, (*COORD[:-1], "method"), method_groups),
        ...
        for key, group in sorted(groups.items(), key=lambda kv: (kv[0][0] != "base", *kv[0][1:-1], METHODS.index(kv[0][-1]))):
            require(sorted(r["seed"] for r in group) == seeds, "group does not contain the ten registered seeds")
            ...
            item.update(expected_seed_count=10, complete_seed_count=len(accepted),
                        tht_eligible_seed_count=len(full),
                        status="COMPLETE_10_SEEDS" if len(accepted) == 10 else "INCOMPLETE_10_SEEDS",
                        tht_status="FULL_POPULATION_10_SEEDS" if len(full) == 10 else "NA_NOT_ALL_TEN_FULL")
            if not paired:
                for name in (*METRICS, "native_wall_seconds"):
                    values = [r.get(name) for r in accepted]
                    item[name] = mean(values) if len(accepted) == 10 and all(v is not None for v in values) else None
                item["affected_edge_count"] = group[0].get("affected_edge_count")
            else:
                item["metrics"] = {}
                for name in METRICS:
                    eligible = len(accepted) == 10 and (name not in TIMINGS or len(full) == 10)
...
def render_report(snapshot):
    ...
    lines = ["# Feng 论文实验套件：状态与结果快照", "",
             ...
             "",
             "**部分进度不能当作全矩阵结论。** 每个组必须齐备预定十种子才显示组平均；"
             "THT 比较还要求双方十种子均全人口完成。不会删除不利种子、使用幸存者队列，或将 2× 一律置 NA。"
             "有任一缺格/失败/证据不一致时，不宣称全矩阵优势。即使全部完成，也须逐指标判断，保留负收益、持平与劣势。",
             "",
             ...
             "",
             "统计粒度：先计算每种子的袋级 min/mean/max，再对预定十种子的对应统计量取算术平均；"
             "下表的 min/max 因而是“种子级 min/max 的平均”，并非把十种子所有袋合并后的极值。"
    ...
    for g in snapshot["method_groups"]:
        ...
        lines.append(f"|{'基础' if g['family']=='base' else '全天故障'}|{desc}|{LABELS[g['method']]}|"
                     f"{g['complete_seed_count']}/10；{g['tht_eligible_seed_count']}/10|{fmt(g['TH'])}|"
                     + "|".join(rates + timing + [fmt(g["native_wall_seconds"])]) + "|")
    lines += ["", "## G31 对各 baseline 的配对结果", "",
              "正值表示 G31 在该指标较好，负值表示较差；胜/平/负按十个逐种子绝对差计算。"
    ...
    for g in snapshot["paired_groups"]:
        ...
        lines.append(f"|{desc}|{LABELS[g['baseline']]}|{g['complete_seed_count']}/10；{g['tht_eligible_seed_count']}/10|"
    ...
    lines += ["", "## 证据与重生成", "",
              ...
              "全部 1760 格、1000 个逐种子 G31/baseline 配对、176 个方法组与 100 个配对组均保留。"
              "快照逐文件读取，开始/结束时间见 JSON；正在运行的格可能在扫描后改变，因此这是进度观察窗口而非全局原子快照。",
              "",
              "```powershell", f'python scripts/eval/report_feng_paper_suite.py --plan "{snapshot["plan_path"]}" --output "新的不存在目录"',
              "```", "",
...
def generate(plan_path, output):
    ...
    snapshot = {"schema": SCHEMA, "status": "COMPLETE" if len(accepted) == 1760 and not pair_mismatch else "PARTIAL_NOT_A_FINAL_CONCLUSION",
                ...
                "expected_cells": 1760, "observed_cell_rows": len(rows), "family_counts": {"base": 480, "all_day_fault": 1280},
                "registered_seeds": seeds, "cell_state_counts": counts, "pair_input_mismatch_count": pair_mismatch,
