...
def main():
    ...
    cases=[(m,s,1000.0) for m in ('map2','nanning') for s in (1.5,2.5,3.0)]+[(m,2.5,1.1) for m in ('map2','nanning')]
    records=[]
    for map_name,speed,horizon in cases:
        name=f'{map_name}_v{speed:g}_h{horizon:g}';payloads={}
        for label in BINARIES:
            target=OUT/'native'/name/label
        ...
        if horizon==1000:
            ...
            assert old['summary']['merge_grant_active_bijection_holds'] is False
        records.append(dict(case=name,map=map_name,speed_mps=speed,horizon_seconds=horizon,equality=equality,
            completed=new['bags'][0]['completed'],finish_time=new['bags'][0]['finish_time'],
    ...
    save(OUT/'verification.json',dict(status='PASS',scope=__doc__,case_count=len(cases),native_runs=2*len(cases),
        binary_identities={k:v[1] for k,v in BINARIES.items()},source_sha256=sha(Path(__file__)),cases=records,
