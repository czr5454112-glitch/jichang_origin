...
def run_case(base: Path, name: str, lines: list[str], *, java: str, classes: Path,
             ...
             expected_error: str | None = None, header: str = "ID Entry STD Start Goal") -> dict:
...
def main() -> int:
    ...
    metadata_lines = [line + " 2 10" for line in direct_lines]
    metadata_header = "ID EntryTime STD star end Unloader Loader"
    metadata_old = run_case(base, "seven_column_direct_old", metadata_lines, java=java, classes=old_classes,
                            repaired=False, threshold=1000, header=metadata_header)
    metadata_new = run_case(base, "seven_column_direct_repaired", metadata_lines, java=java, classes=classes,
                            threshold=1000, header=metadata_header)
    cases += [metadata_old, metadata_new]
    metadata_identical = {}
    for name in ("release.csv", "routes.csv", "output.txt", "outputstarttime.txt"):
        before, after = Path(metadata_old["path"]) / name, Path(metadata_new["path"]) / name
        require(before.read_bytes() == after.read_bytes() == (Path(direct_new["path"]) / name).read_bytes(),
                "seven-column legacy metadata compatibility changed: " + name)
        metadata_identical[name] = sha(before)
    for name, value in metadata_old["summary"].items():
        require(metadata_new["summary"][name] == value, "seven-column direct summary differs: " + name)
    require(metadata_new["mapping"] == direct_new["mapping"], "metadata altered execution mapping")
    checks.append({"name": "real_seven_column_raw_schema_ignores_metadata_matches_old_direct_bytes",
                   "pass": True, "input_header": metadata_header, "identical_files": metadata_identical})
