...
def cleanup_epoch_scratch(output: Path) -> dict:
    ...
    if scratch.exists():
        ...
        for item in entries:
            require(item.is_file() and not item.is_symlink() and item.resolve(strict=True).parent == scratch
                    and not (item.lstat().st_file_attributes & stat.FILE_ATTRIBUTE_REPARSE_POINT
                    if hasattr(item.lstat(), "st_file_attributes") else False), "non-flat/reparse scratch entry; retain for manual inspection")
