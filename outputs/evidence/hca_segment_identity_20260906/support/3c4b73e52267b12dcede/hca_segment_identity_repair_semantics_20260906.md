...
首轮真实预检暴露了新 wrapper 额外引入的输入兼容性错误：它要求每行恰好五列，而冻结 raw 文件合法地保留了 `ID EntryTime STD star end Unloader Loader` 七列。map2 1.75× / seed 104729，以及南宁 1× / seeds 155921、181081、232003 四个进程均在 `readTaskList` 阶段报 `expected five raw input columns` 并以返回码 1 退出；逐格确认均无 `summary.csv` 或 `normalized_result.json`，没有可纳入性能统计的成功数值结果。修正仅将新增的 `order.length != 5` 拒绝条件改为 `order.length < 5`，恢复旧 wrapper 读取前五列并忽略后缀元数据的兼容行为；没有改 raw、canonical、路径算法、EBS 合同或运行时域。这不是依据相对性能选择实现。

被拒绝尝试的[四个失败格、旧 freeze 与执行记录](../../outputs/runtime/hca_segment_identity_20260906/preflight/superseded_five_column_attempt_20260906/)、[原语义文档](../../outputs/runtime/hca_segment_identity_20260906/preflight/superseded_five_column_attempt_20260906/semantics_before_input_compatibility.md)以及[旧 wrapper、六项微测与 class 保存清单](../../outputs/runtime/hca_segment_identity_20260906/preflight/superseded_five_column_input/preservation_manifest.json)均独立保留。原文档 SHA256 为 `27b39dae4ba77f3facce16ec252e65b52f756202ec589a134b72807cb6fe08ad`；被替代 wrapper SHA256 为 `041899096be424ca15da1d621131e1e6cba078c897770e1d32158c208d67ffd0`，不混入修正后的正式矩阵。

输入兼容修正后的最终 wrapper SHA256 为 `3333729ba81c2e74ac98519e52b161a4b1fbbe37148e9f7c661690fcf28dfbbd`；首轮兼容输入预检使用的单格 runner SHA256 为 `77105ac1695576011118e8fc2d3d450e26149cc5a165728fae7962bb4fd46230`，其[原源码](../../outputs/runtime/hca_segment_identity_20260906/preflight/superseded_slow_cleanup_tools/run_hca_segment_identity.py)继续保留。runner 以 execution ID 精确关联全部事件、原生终态成员及 canonical 段，不做旧 raw-ID FIFO completion 匹配；已修正上述第三列时钟误认。计时从共同 D 重算，未单独增加 EBS 依赖。正式完整 source/class 集合由修正后的构建与 campaign 冻结记录绑定，不能沿用被替代 wrapper 的旧编译身份。

修正后正式构建位于 `build/hca_segment_identity_v1_input_compat`：source 集合主聚合 SHA256 为 `74c1a899b879b0523d039e9fa650ef3c860557c7fc96e80ff3a8978ae3faf92c`，class 集合主聚合 SHA256 为 `1fbd4c494aa21a34380ddcde6606a3a0d93883533144f0546a903ccf04a872b3`。该聚合使用正式 runner 的文件路径及哈希编码，不与微测试工具的另一种内容聚合格式混用。

[七项原生微测试](../../outputs/runtime/hca_segment_identity_20260906/preflight/microtests.json)全部通过，记录 SHA256 为 `cfcefa2bf05fb9438b8074de48b4c545b318ac86f23508d78249337c12a29628`。重叠案例记录两段均在 epoch 1 释放：入库完成 21，出库完成 3；旧执行只留下一个完成，新执行两段均完成。普通三袋单段案例包含分数 D，四份原生日志逐字节相同。新增七列 raw 案例的 `release.csv`、`routes.csv`、`output.txt`、`outputstarttime.txt` 与旧 wrapper 对照及五列新 wrapper 案例逐字节一致，旧 summary 字段及执行段映射也一致。当前重叠案例两条路线使用不同节点，直接证明的是段身份、活动路径及完成事件隔离；同节点预约使用独立键的结论另有 `update_constrain/Contains` 的直接源码依据，不能把该案例夸称穷尽验证了共享节点预约或连续时间碰撞自由。

另有[共享节点预约身份补充验证](../../outputs/runtime/hca_segment_identity_20260906/preflight/reservation_audit.json)，已在上述 input-compatible 正式生产类上重做通过：两执行 ID 在节点 17、18 的预约并存，更新其中一个保留另一个的对象和区间，同 ID 负对照则覆盖旧预约。这是实际 `update_constrain/Contains` 的存储隔离验证，不是路线可行性或物理碰撞自由证明，也不替代上述七项原生微测试。

其后只优化了 Python `cleanup_epoch_scratch` 对已归档 scratch 文件重复解析路径的开销。最终 [`run_hca_segment_identity.py`](../../scripts/eval/run_hca_segment_identity.py) SHA256 为 `9241cf23d4d7e3cc699fde4f812f6569cee9ff239c9e4f4393fbe4b43b61c525`；[清理修订验证](../../outputs/runtime/hca_segment_identity_20260906/preflight/cleanup_revision_validation.json)记录其余模块 AST 完全一致、21 项测试通过，以及小样原生/派生数据、gzip 和归档 manifest 字节一致。100 文件的元数据检查计时只测文件系统清理前检查，不能当作完整运行或算法加速比。Java source/class、数值计算、输入、时域、协议和原生预约算法均保持上述冻结身份。

统一最终 60 格写入 `outputs/runtime/hca_segment_identity_20260906_fast_cleanup`；原 `outputs/runtime/hca_segment_identity_20260906` 中五个慢清理预检保留为工具版本对照，不计入最终 60 格。对应格的完整数值/事件比较以独立验证为准，不能仅以小样清理测试推定全人口已经逐字相同。这个变更属于文件清理执行开销优化，不是新的路由算法版本，也不依据相对性能选择结果。首格已有[源端等待独立诊断](hca_segment_identity_release_delay_diagnostic_20260906.md)，其单格结论不外推到全矩阵。
