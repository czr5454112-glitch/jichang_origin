...
Reads existing native logs; never executes Java, modifies a simulation input,
or changes a frozen result. Only the requested separate diagnostic JSON is
written. The common completed cohort is diagnostic, never a formal THT estimate.
"""
...
def diagnose(new=NEW, old=OLD, old_table=OLD_TABLE):
    ...
    identity_path = Path(normalized["workload_identity_path"])
    ...
    old_row = old_rows[0]
    actual_inputs = {k: sha(Path(identity[k+"_path"])) for k in ("raw", "canonical", "map")}
    ...
    evidence_paths = [Path(__file__), old_table, identity_path, *[Path(identity[k+"_path"]) for k in ("raw", "canonical", "map")],
