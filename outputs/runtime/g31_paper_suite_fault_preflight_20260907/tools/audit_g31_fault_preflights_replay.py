...
FROZEN_RUNNER = OUTPUT / "tools/run_g31_tarau_paper_suite.py"
FROZEN_RUNNER_SHA = "72fd23feaa8fde3ee735571beca40277a053180246246becf7447e20d225785b"
...
def source_binding():
    assert sha(FROZEN_RUNNER) == FROZEN_RUNNER_SHA
    def scientific_ast(path):
        tree = ast.parse(path.read_text(encoding="utf-8-sig"))
        matches = [node for node in tree.body if isinstance(node, ast.FunctionDef) and node.name == "load_completed"]
        assert len(matches) == 1
        matches[0].body = [ast.Pass()]
        return ast.dump(tree, include_attributes=False)
    current = Path(runner.__file__)
    assert scientific_ast(FROZEN_RUNNER) == scientific_ast(current), "source changed outside the read-only loader"
    return {"status": "PASS", "frozen_native_runner_path": str(FROZEN_RUNNER),
        "frozen_native_runner_sha256": sha(FROZEN_RUNNER), "current_loader_path": str(current),
        "current_loader_sha256": sha(current), "ast_identical_except_load_completed": True,
        "scope": "Exact syntax-tree equality for all functions and module constants except the existing read-only load_completed body; no native algorithm or request-building change accepted."}


def check_provenance(result):
    source_binding()
    prepared = runner.prepare_spec(result["spec"])
    expected = dict(prepared.provenance)
    expected["runner_sha256"] = FROZEN_RUNNER_SHA
    assert runner._encode(result["provenance"]) == runner._encode(expected), "actual input/source/binary provenance mismatch"
    return prepared
...
def verified_result(directory):
    ...
    prepared = check_provenance(result)
    decoded = {}
