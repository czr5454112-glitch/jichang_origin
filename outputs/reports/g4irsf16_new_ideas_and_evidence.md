...
## 15. Use the sealed formal-release contract, not a successor patch

**Verified predecessor finding.** After EOL reconstruction, the unchanged G4IRSF15 validator passed the 747,962-row descriptor scan, the 192-attempt round-1 explicit-blocker check, and the 64-attempt round-2 pilot. Formal validation then reached two historical generator/validator boundary cases: the formal plan stores `pilot_round: null`, and six of 2,172 labels legitimately bind an observed but empty I4/H_system realized-delta set. These are neither evidence drift nor failed scientific gates.

**Verified compatibility change.** The G4IRSF15 seal already contains its official `validate_g4irsf15_formal_release.py` entry point and ten focused regressions. Successor CI now executes that exact sealed entry point from the disposable seal worktree. It shallow-copies the plan only for compact-path resolution, mapping `null` to round 1 without changing the disk plan, self-hash, or 476 evidence bindings. It accepts a zero-row binding only for the exact I4/H_system empty-realized-set representation while independently checking the empty-content SHA, direct/realized/external arrays, and cohort sidecar. Temporary validator adaptation is restored after the call. No G4IRSF16 source patch or exception skip is retained.

**Independent verification.** The official wrapper regressions pass 10/10. Under that wrapper, all 2,172 labels, 2,072 split groups, 142 weighted-effect estimates, and nine response summaries independently validate; no third nullable, empty-set, or integer-lower-bound conflict was found.

**General lesson.** A frozen evidence bundle should include and reuse its own executable compatibility contract. Successors should prefer a sealed, narrowly tested release entry point over reimplementing historical semantics, even when both can be made fail-closed.
