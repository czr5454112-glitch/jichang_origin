...
## 15. Sealed validators need explicit null semantics

**Verified predecessor finding.** After EOL reconstruction, the unchanged G4IRSF15 validator passed the 747,962-row descriptor scan, the 192-attempt round-1 explicit-blocker check, and the 64-attempt round-2 pilot. Formal validation then reached compact evidence and raised `TypeError` because the formal plan correctly stores `pilot_round: null`, while the validator evaluated `int(plan.get("pilot_round", 1))`. An existing null does not use the mapping default. This is a validator control-flow defect, not evidence drift or a failed scientific gate.

**Verified compatibility change.** G4IRSF16 formal CI loads the exact sealed validator bytes by pinned SHA-256, applies one in-memory replacement that maps only `None` to the formal path default, and invokes the original formal validator. The sealed script, plan, artifacts, and disposable worktree remain unchanged on disk. Any validator-byte or patch-context drift fails closed. A regression proves the formal null proceeds to the original downstream binding checks, while a tampered validator is rejected.

**General lesson.** A frozen evidence bundle should include executable tests for nullable schema fields, especially when `null` and missing have different meanings. Compatibility fixes must pin both the original program identity and the exact semantic delta rather than broadly catching exceptions or skipping validation.
