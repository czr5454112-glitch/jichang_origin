...
Execution status: **STOPPED / INCOMPLETE**. The external-robustness campaign contains
`166/180` normalized cells. Scientific validity is separate: all 16 old
Nanning DH results are `INVALIDATED_ZERO_THROUGH_STATE_MACHINE_BUG` and are
excluded from formal comparisons. All `90/90` map2 cells and all `60/60`
...
## Defective-program observations preserved at the checkpoint

| map/load | terminal CIE-DH seeds, scientifically invalid | mean completed | completion rate | mean on-time | on-time rate | formal population latency |
|---|---:|---:|---:|---:|---:|---|
...
cells complete 24,473–24,484. These approximately 44% completion observations
come from a program with a confirmed zero-through intermediate-node state
transition bug. They are withdrawn as evidence of the valid reconstruction's
Nanning capability, normal congestion, or Feng's original CIE-DH. The precise
share of the completion deficit caused by this bug has not been quantified.
...
admission-to-completion duration (summed over segments per raw bag), but 15.32%
higher P95, 25.19% higher P99, and 42.07% higher
...
measured simulator wall time is 582.73x longer. These are contaminated-program
map-to-map ratios, not G31 speedups. Their previous normal-congestion scaling
interpretation is withdrawn; they do not quantify the bug's full effect.
...
At the checkpoint, an operating-system shutdown would have interrupted only
the twelve in-flight cells; the sixteen completed files remain complete as
execution records. Their scientific validity is separately revoked above.
...
The final stopped boundary is therefore `16` terminal complete files and `14`
...
repair, preserve all old files, retire this defective campaign, and use a new
version directory for every corrected Nanning coordinate. None of the sixteen
old terminal cells may fill a corrected-matrix slot. Partial native output
must never be normalized or counted. The immutable execution-stop source is
commit `05f3edc6b14791dd87f79c143d1bd4084b24954b`.
...
## Scientific disposition after the independent audit

The old Nanning campaign is retired due to the confirmed implementation bug.
The complete map2 10-seed evidence remains versioned evidence pending the
required corrected-source per-bag regression. The historical Table 5.3 anchor,
HCA/G31 native runs and their factorial/ablation evidence are not invalidated
by this independent Java bug. The exclusion sidecar is
