...
robustness matrix is explicitly incomplete at `161/180` normalized cells;
Nanning CIE-DH contributes `11/30` cells and no final cross-map claim is made.
...
10. **Do external paired seeds support G31 against reconstructed DH?** The
    ...
    2026-09-05 shutdown checkpoint, `161/180` normalized cells exist: map2 is
    complete and mixed at 1x (G31 mean is lower, but its P95/P99/max are
    higher), whereas the Nanning DH port has only `11/30` cells. Its seven 1x
    cells complete 44.530% on average and its four 1.75x cells complete
    44.308% on average, with formal population timing unavailable because
    every cell is incomplete. These are interim diagnostics, not a final
    external ranking.
