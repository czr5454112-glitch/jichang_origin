...
- G3g Legacy scheduler semantics alignment: `outputs/reports/g3g_legacy_scheduler_semantics_alignment_report.md` compares Legacy node-window full-route scheduling, strict G3f local executable replay, and full-route SIPP runtime scheduling. All `29/29` G3f unresolved capacity cases are current-node hold-capacity failures; Legacy node-window full-route reaches `127/144` but creates `458` real edge/merge conflicts, while SIPP full-route reaches `144/144` with zero conflicts. This confirms a scheduler-scope/backpressure semantics gap, not a green light for G4A/training.
