...
- Java/C++ legacy no-fault scheduler-window gate: an external headless harness calls read-only Java `ICS_PathFinding`, while native C++ mirrors task arrival, active-route advancement, constraint rebuilds, unfinished retries, and A* planning on the same `map2/inputdata` window; planned route multiset parity passes and Release C++ is faster on the recorded benchmark.
