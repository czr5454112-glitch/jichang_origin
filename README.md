...
- G3i CIE/A* path-constrained SIPP integration: `outputs/reports/g3i_cie_sipp_integration_report.md` adds `LegacyRouteSIPPPlanner` / `LegacyRouteSIPPBaseline`, where original CIE/Legacy A* chooses the fixed route and SIPP-style timing only decides when that route can safely use occupied nodes, edges, and merge groups. The real `map2/inputdata` matched-window replay reaches `132/144` with `0` real conflicts, and all `132/132` planned routes keep the A* path; this is a G4A pilot candidate after auditing the remaining `12` CIE no-path cases.
