...
- G3k CIE node-window retry audit: `outputs/reports/g3k_cie_node_window_retry_audit_report.md` mirrors the original Java `unfinishTasks` retry behavior under the verified CIE/A* node-window scope. It reproduces G3j primary (`127/144`, `0` node-window conflicts), then recovers all `17/17` G3j no-path cases with source-wait retry, reaching `144/144` planned and `0` node-window conflicts without applying `edge_capacity=1`; edge overlaps (`556`) remain diagnostic only. This makes a G4A pilot dataset candidate under verified CIE/Java retry semantics, not a training result.
- G4A/G4B verified CIE retry policy pilot: `outputs/reports/g4a_cie_retry_teacher_dataset_report.md` converts the G3k teacher from route-level rows into `1186` per-interface `MOVE_TO_NEXT_CIE` slices plus `17` source-retry slices, with forbidden-feature leakage checks and reproducible train/val/test splits. `outputs/reports/g4b_cie_retry_policy_pilot_report.md` trains a minimal MLP candidate scorer from allowed runtime features only; offline top-1 is `0.988` versus `0.856` for the shortest-time heuristic, shadow replay has `14/1186` disagreements, and conservative route-exact closed-loop planning reaches `132/144` with `0` node-window conflicts. This is a G4C data-aggregation candidate, not a paper-grade learning success or RL result.
- G4C failure-driven data aggregation: `outputs/reports/g4c_failure_driven_data_aggregation_report.md` and `outputs/reports/g4c_learner_visited_closed_loop_report.md` keep `scenario` metadata-only, cluster the `14` G4B high-confidence interface disagreements into `4` risky branch clusters, relabel `14` learner-visited off-route states with the verified CIE retry teacher, and train a minimal round1 scorer with calibrated abstain. Cluster abstain calls the verified CIE/A* fallback on `114/1186` interface decisions (`9.6%` fallback, `90.4%` A* calls saved), reaches `144/144` planned, keeps node-window conflicts at `0`, and reduces wrong high-confidence actions from `14` to `0`. This passes the G4D large-window expansion gate, not an RL or paper-grade replacement claim.
- G4D large-window runtime replacement audit: `outputs/reports/g4d_large_window_teacher_dataset_report.md` expands the verified CIE retry teacher to `8` real inputdata windows from `144` through `1024` tasks (`4496` window task rows total), producing `39313` MOVE slices plus `962` source-retry slices while keeping node-window conflicts at `0` and preserving `edge_capacity=1` as diagnostic-only. `outputs/reports/g4d_policy_training_report.md` trains a small enhanced MLP scorer plus calibrated risk head, not RL/GNN/Transformer; all-split top-1 is `0.955` versus `0.861` for shortest-time. `outputs/reports/g4d_true_closed_loop_report.md` shows G4D matches the verified teacher planned scope (`4449/4496`; the remaining `47` are teacher no-path rows in one high-density 1024 window), reduces wrong high-confidence actions to `0`, and cuts aggregate verified CIE/A* calls from `15852` to `6786` (`57.2%` reduction). Several small windows still have per-window A* call regressions because fallback is conservative, so this is a G4E/C++ runtime-evaluation candidate, not a final replacement claim.
