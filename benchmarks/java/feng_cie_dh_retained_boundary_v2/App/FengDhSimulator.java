...
public final class FengDhSimulator {
    ...
    public long step(int traceSampleModulo) {
        ...
        ArrayList<FengDhBagState> retainedTransferWaiting = new ArrayList<FengDhBagState>();
            ...
            for (FengDhBagState bag : lattice.occupantsDownstreamFirst(edge.id)) {
                ...
                if (bag.isRetainedBoundaryTransfer()) {
                    if (!bag.isNodeServiceReadyAt(commitTick)) {
                        retainedTransferWaiting.add(bag);
                        progress++;
                        continue;
                    }
                    servicesFinishing.add(bag);
                    FengDhPolicy.Decision decision = policy.choose(edge.to, bag.goalNode, snapshot);
                    if (decision == null) {
                        stopNoPath(bag, commitTick, edge.to, traceSampleModulo);
                    } else {
                        ...
                        addEntry(entriesByEdge, new EntryProposal(bag, edge.id, edge.to, decision));
                    }
                    continue;
                }
        ...
        // A finishing through service frees only its node-wide service identity.
        // Its incoming-edge footprint remains through transfer and outlet HOLD.
        ...
        // Source induction remains unchanged. Intermediate transfer timers are
        // handled above on their incoming edge, never as occupancy-free nodes.
        ...
        // Resolve entries with fixed FIFO winners. Each accepted boundary transfer
        // proves its upstream vacancy; monotone re-planning exposes that vacancy
        // to other entries in the same commit. A blocked cycle has no initial
        // guaranteed vacancy and is not globally rotated through occupied cells.
        LinkedHashMap<Long, EntryProposal> approvedEntries =
                new LinkedHashMap<Long, EntryProposal>();
        HashSet<Integer> approvedTargetEdges = new HashSet<Integer>();
        for (ArrayList<EntryProposal> proposals : entriesByEdge.values()) {
            Collections.sort(proposals, entryOrder());
        }
        boolean newBoundaryDeparture;
        do {
            newBoundaryDeparture = false;
            for (FengDhEdgeLattice.EdgeData target : lattice.edges()) {
                ...
                if (proposals == null || proposals.isEmpty()
                        || approvedTargetEdges.contains(Integer.valueOf(target.id))) {
                    continue;
                }
                if (lattice.entryBlockerAfterPlan(
                        target.id, plannedPositions, guaranteedDepartures) != null) {
                    continue;
                }
                EntryProposal winner = proposals.get(0);
                approvedEntries.put(Long.valueOf(winner.bag.taskId), winner);
                approvedTargetEdges.add(Integer.valueOf(target.id));
                if (winner.upstreamEdgeId >= 0) {
                    guaranteedDepartures.add(Long.valueOf(winner.bag.taskId));
                    newBoundaryDeparture = true;
                }
            }
            if (newBoundaryDeparture) {
                plannedPositions.clear();
                internalMoves.clear();
                internalStops.clear();
                planInternalMovement(guaranteedDepartures, plannedPositions, internalMoves, internalStops);
            }
        } while (newBoundaryDeparture);
        for (FengDhEdgeLattice.EdgeData target : lattice.edges()) {
            ArrayList<EntryProposal> proposals = entriesByEdge.get(Integer.valueOf(target.id));
            if (proposals == null || proposals.isEmpty()) {
                continue;
            }
            if (approvedTargetEdges.contains(Integer.valueOf(target.id))) {
                for (int index = 1; index < proposals.size(); index++) {
                    holdForMergeConflict(proposals.get(index), proposals.get(0), commitTick, traceSampleModulo);
                }
            } else {
                FengDhEdgeLattice.EntryBlocker blocker = lattice.entryBlockerAfterPlan(
                        target.id, plannedPositions, guaranteedDepartures);
                if (blocker == null) {
                    throw new IllegalStateException("unresolved free target after transfer closure");
                }
        ...
        // Zero through-time advances once to the retained per-bag transfer.
        // Neither service start nor timer expiry invents space off the lattice.
        for (NodeServiceProposal proposal : approvedNodeServices.values()) {
            ...
            bag.beginRetainedBoundaryTransfer(
                    commitTick,
                    ...
                    shouldTrace(bag, traceSampleModulo));
            stopPhysicalHandoff(bag, commitTick, "RETAINED_BOUNDARY_TRANSFER", traceSampleModulo);
            progress++;
        }

        // Remove only actual goal/edge departures before follower mutations.
        // Finishing positive through services continue to occupy their edge.
        ...
        for (EntryProposal proposal : approvedEntries.values()) {
            if (proposal.upstreamEdgeId >= 0) {
                lattice.remove(proposal.upstreamEdgeId, proposal.bag);
            }
        }
        ...
        for (FengDhBagState bag : retainedTransferWaiting) {
            stopPhysicalHandoff(bag, commitTick, "RETAINED_BOUNDARY_TRANSFER", traceSampleModulo);
        }
        ...
        for (NodeServiceProposal proposal : approvedNodeServices.values()) {
            ...
            if (throughTicks == 0L) {
                continue; // Advanced once to retained transfer earlier in this commit.
            }
