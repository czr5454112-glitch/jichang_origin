...
public final class FengDhBenchmark {
    private static final String METHOD = "FENG_DH_PAPER_SUITE_V6";
    ...
    private static void runStaticBridge(HashMap<String, String> options) throws Exception {
        ...
        FengDhEdgeLattice lattice = FengDhEdgeLattice.readLegacyMap(
                mapPath, doubleOption(options, "--speed-mps", 2.5d));
        ...
        System.out.println("method=STATIC_FREE_FLOW_PAPER_SUITE_V6"
    ...
    private static void runFormal(HashMap<String, String> options) throws Exception {
        ...
        lattice.setInitialBlockedEdges(options.containsKey("--blocked-edges")
                ? options.get("--blocked-edges") : "");
        ...
        FengDhSimulator.RunResult result = simulator.run(
                new FengDhSimulator.RunConfig(horizonTick, traceSampleModulo, Long.MAX_VALUE));
        ...
        String executedMethod = isStaticControl(policy)
                ? "STATIC_FREE_FLOW_PAPER_SUITE_V6" : METHOD;
    ...
    private static void writeSummary(
        ...
        add(header, values, "method",
                staticControl ? "STATIC_FREE_FLOW_PAPER_SUITE_V6" : METHOD);
        ...
        add(header, values, "cell_meters", lattice.getCellMeters());
        add(header, values, "speed_meters_per_second",
                lattice.getSpeedMetersPerSecond());
        add(header, values, "initial_blocked_directed_edge_count", lattice.getBlockedEdgeCount());
    ...
    private static void writeSegments(File path, List<FengDhBagState> segments) throws IOException {
        ...
        try {
            writer.write("task_id,raw_bag_id,source_raw_bag_id,segment_id,segment_count,start,goal,"
                    ...
                    + "last_hold_reason,final_node,current_edge_id,position_cell");
            writer.newLine();
            for (FengDhBagState bag : segments) {
