...
public final class FengDhSimulator {
        ...
        for (FengDhEdgeLattice.EdgeData edge : lattice.edges()) {
            for (FengDhBagState bag : lattice.occupantsDownstreamFirst(edge.id)) {
                ...
                if (throughOccupant == bag) {
                    ...
                    // Test HOLD before leaving upstream, preserving overlapping transit.
    ...
    private boolean outletStopped(FengDhBagState bag, int node,
            FengDhEdgeLattice.Snapshot snapshot) {
        FengDhPolicy.Decision decision = policy.choose(node, bag.goalNode, snapshot);
        if (decision == null) return true;
        // Earlier zero-through HOLD proposals must not leak their status
